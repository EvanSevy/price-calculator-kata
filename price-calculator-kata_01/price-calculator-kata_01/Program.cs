...
namespace price_calculator_kata_01
{
    class Program
    {
        static void Main(string[] args)
        {
			...
			AddedExpense Packaging = new AddedExpense() { Name = "Packaging", Value = 5m, Type = ExpenseType.Percentage };
			AddedExpense BubbleWrap = new AddedExpense() { Name = "Bubble Wrap", Value = 2m, Type = ExpenseType.Percentage };
			...
			Product chair = new Product("Aeron Chair", 34567, 1000m);
			products.Add(ProductPriceFactory.ForProduct(chair).WithTaxRate(20m).WithDiscountRate(15m).GetFactoryResult());

			Product shoes = new Product("Salomon Trail Runners", 123, 145m);
			products.Add(ProductPriceFactory.ForProduct(shoes).WithTaxRate(7m).WithDiscountRate(15m).WithAddedExpense(BubbleWrap).GetFactoryResult());
